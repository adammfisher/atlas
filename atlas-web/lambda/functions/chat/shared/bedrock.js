 ...
 */
async function compactConversation(messages, cachedSummary = null) {
  ...
  const tiers = {
    ...
    middle: messages.slice(-30, -10),
    old: messages.slice(0, -30)
  };
  ...
  const totalOldCount = tiers.old.length + tiers.middle.length;
  if (cachedSummary && cachedSummary.messageCount === totalOldCount) {
    keyPoints = cachedSummary.keyPoints;
    middleSummary = cachedSummary.middleSummary;
  } else {
    // Generate new summaries
    if (tiers.middle.length > 0) {
      middleSummary = await summarizeMessages(tiers.middle);
    }
    if (tiers.old.length > 0) {
      keyPoints = await extractKeyPoints(tiers.old);
    }
  }
